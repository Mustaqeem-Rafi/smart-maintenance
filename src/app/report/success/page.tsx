...
import { CheckCircle, Copy, Check, LayoutDashboard, FileText } from "lucide-react";
...
function SuccessContent() {
  ...
  const handleDashboardRedirect = () => {
    ...
    else if (role === 'technician') router.push('/staff/dashboard');
    else router.push('/student/dashboard');
  };

  const handleHistoryRedirect = () => {
    const role = (session?.user as any)?.role;
    if (role === 'admin') router.push('/admin/incidents');
    else if (role === 'technician') router.push('/staff/reports');
    else router.push('/student/history'); // Or dashboard if you don't have a separate history page
  };
    ...
    <div className="flex flex-col items-center justify-start rounded-xl shadow-lg bg-white border border-gray-200 overflow-hidden max-w-2xl w-full mx-auto mt-10">
      ...
      <div className="flex flex-col items-center justify-center gap-4 p-8 text-center w-full">
        ...
        <p className="text-gray-500 text-base font-normal leading-normal max-w-md">
          Thank you. Your report has been sent to the relevant department for review.
        </p>
      </div>
      ...
      <div className="w-full border-t border-gray-200 p-6 md:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-lg bg-gray-50 border border-gray-200 mb-6">
          ...
          <button onClick={handleCopy} className="flex items-center justify-center gap-2 text-sm font-medium text-blue-600 hover:text-blue-700">
      ...
      <div className="w-full flex flex-col sm:flex-row-reverse justify-stretch gap-3 px-6 md:px-8 py-6 border-t border-gray-200 bg-gray-50">
        <button 
          ...
          className="flex w-full sm:w-auto min-w-[84px] items-center justify-center rounded-lg h-10 px-5 bg-blue-600 text-white text-sm font-bold hover:bg-blue-700 transition gap-2"
        >
          <FileText className="w-4 h-4" /> View My Reported Incident
        </button>
        <button 
          onClick={handleDashboardRedirect}
          className="flex w-full sm:w-auto min-w-[84px] items-center justify-center rounded-lg h-10 px-5 bg-transparent text-blue-600 text-sm font-bold border border-blue-600 hover:bg-blue-50 transition gap-2"
        >
...
export default function IncidentSuccessPage() {
  return (
    <div className="min-h-screen bg-gray-50 p-4 font-sans flex flex-col items-center">
      <Suspense fallback={<div className="flex h-[50vh] items-center justify-center"><div className="w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div></div>}>
        <SuccessContent />
      </Suspense>
    </div>
  );
